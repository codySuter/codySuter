// Ace Bay Studio — Electron main process.
// The whole bay map is one JSON file in userData (map.json). A rotating
// automatic backup is written on every quit, same safety net as the
// other Ace Studio apps.
...
const VERSION_URL =
  'https://github.com/codysuter/codysuter/releases/download/ace-bay-studio-windows/version.txt';
const RELEASE_PAGE = 'https://github.com/codysuter/codysuter/releases/tag/ace-bay-studio-windows';
const AUTO_BACKUPS_KEPT = 15;

let mainWindow = null;

const mapFile = () => path.join(app.getPath('userData'), 'map.json');

function readMapSync() {
  try {
    return JSON.parse(fs.readFileSync(mapFile(), 'utf8'));
  } catch {
    return null; // First run, or an unreadable file — the renderer seeds a fresh map.
  }
}

ipcMain.handle('map:load', async () => readMapSync());

ipcMain.handle('map:save', async (_e, map) => {
  if (!map || !Array.isArray(map.aisles)) throw new Error('Invalid map');
...
// ---- file dialogs (CSV contents import, JSON backup/restore) ----

const FILTERS = {
  csv: [{ name: 'CSV spreadsheets', extensions: ['csv'] }],
...
ipcMain.handle('file:pick', async (_e, kind) => {
  try {
    const { canceled, filePaths } = await dialog.showOpenDialog(mainWindow, {
      title: kind === 'csv' ? 'Import OPTI contents (CSV)' : 'Restore from backup',
      filters: FILTERS[kind] || FILTERS.json,
      properties: ['openFile'],
    });
    if (canceled || filePaths.length === 0) return { ok: false, canceled: true };
    const text = await fsp.readFile(filePaths[0], 'utf8');
    return { ok: true, name: path.basename(filePaths[0]), text };
  } catch (err) {
...
// Rotating automatic backup of the map on every quit, in userData/backups.
function autoBackup() {
  try {
    ...
    fs.writeFileSync(path.join(dir, `auto-${stamp}.json`), JSON.stringify(map, null, 2), 'utf8');
    const autos = fs
      .readdirSync(dir)
...
function supportMailto(kind) {
  ...
  const subject = `Ace Bay Studio ${app.getVersion()} — ${label}`;
  ...
  try {
    ...
    if (newerVersion(latest, app.getVersion())) {
      const { response } = await dialog.showMessageBox(mainWindow, {
        ...
        detail: 'The download replaces AceBayStudio.exe. Your bay map lives in its own folder and is untouched.',
  ...
  const template = [
    {
      label: 'File',
      submenu: [
        { label: 'Import contents (CSV)…', accelerator: 'CmdOrCtrl+I', click: () => sendMenu('import-csv') },
        { label: 'Download CSV template', click: () => sendMenu('csv-template') },
        { type: 'separator' },
        { label: 'Back up bay map…', accelerator: 'CmdOrCtrl+B', click: () => sendMenu('backup') },
